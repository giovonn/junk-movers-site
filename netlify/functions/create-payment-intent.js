...
// Sub payout rules:
// - Standard split is 50/50 on every tier except Full Load.
// - Full Load pays the sub 55% (physically hardest job, closest to under-market vs. real hauler rates).
// - No payout ever falls below $100, regardless of split — protects small jobs from being too thin for subs to accept.
const MIN_SUB_PAYOUT = 10000; // $100.00, in cents
function calcSubPayout(tier, clientAmountCents) {
  const splitRate = tier === 'full' ? 0.55 : 0.50;
  const raw = Math.round(clientAmountCents * splitRate);
  return Math.max(raw, MIN_SUB_PAYOUT);
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const subPayout = calcSubPayout(tier, quotedAmount);
    const yourCut = quotedAmount - subPayout;
